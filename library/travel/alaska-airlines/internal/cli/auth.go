...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Alaska Airlines",
	}
...
Use --chrome to read cookies from Chrome for alaskaair.com.
...
profile by name when the installed backend supports it.`,
		Example: `  alaska-airlines-pp-cli auth login --chrome
  alaska-airlines-pp-cli auth login --browser
  alaska-airlines-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  alaska-airlines-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  alaska-airlines-pp-cli auth login --browser\n")
				return nil
			}
			...
			requiredCookies := []string{"AS_ACNT", "AS_NAME", "guestsession", "guestidentity", "ASSession", "ASSessionSSL", "as_pers"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  alaska-airlines-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  alaska-airlines-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: alaskaair.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  alaska-airlines-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func validateComposedAuth(authHeader string) error {
	testURL := "https://www.alaskaair.com"
