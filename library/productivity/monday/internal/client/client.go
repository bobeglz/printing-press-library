...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "monday-pp-cli")
...
func (c *Client) cacheKey(path string, params map[string]string) string {
	key := path
	// Sort keys before concatenating: Go map iteration order is randomized
	// per run, so an unsorted walk would hash identical requests to different
	// cache keys, guaranteeing misses and unbounded duplicate cache files.
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "monday-pp-cli/2026-04")
		}
...
func (c *Client) refreshAccessToken() error {
	...
	// monday.com OAuth token endpoint. Only reached when OAuth tokens are
	// configured (AccessToken + RefreshToken + expiry set via SaveTokens);
	// the default api_key auth flow leaves those fields empty, so this path
	// stays inert for token-based users while still refreshing real OAuth
	// sessions transparently instead of failing silently on expiry.
